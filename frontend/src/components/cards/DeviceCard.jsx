    ...
    <div className="bg-white rounded-xl p-5 shadow-md border border-gray-100 hover:shadow-lg transition-shadow">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-xs text-gray-400 uppercase tracking-wider">{device.type} | {device.location}</p>
        </div>
        ...
        <button 
          ...
          className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase cursor-pointer hover:opacity-80 transition-opacity ${
            !isOnline ? "bg-red-50 text-red-400 cursor-not-allowed" : isOn ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"
          }`}
        >
          {isOn ? "● ON" : "○ OFF"}
        </button>
